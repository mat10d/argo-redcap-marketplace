...
Repository rename table, the Data Access Groups, the user→role table, and the token-optional
build_tracking commands directly from the SIR. Replaces hand-writing the brief each build.

Token-optional ([[token-optional]]): pulls the SIR via the API if a token is present, or reads a
pre-pulled record JSON (`sir_update.py <RID> --pull > rec.json`) with --from-json.

Usage:
    set -a; source ~/.argo/.env; set +a
    python3 setup_brief.py <RID> --out <build_dir> [--moniker HPV_SelfSampling]
    python3 setup_brief.py <RID> --from-json rec.json --out <build_dir> --moniker HPV_SelfSampling
"""
...
def main():
    ...
    ap.add_argument("rid"); ap.add_argument("--out",required=True,help="Build folder to write the brief into")
    ap.add_argument("--moniker",help="Study moniker for File Repository renames (default: PI surname)")
    ap.add_argument("--from-json",help="Use a pre-pulled record JSON instead of the API (no-token mode)")
    a=ap.parse_args()
    ...
    L.append("\nToken-optional: mark `build_tracking` via `sir_update.py` if you have the SIR token, "
             "else set the fields in the Study Tracker UI.\n")
    ...
    L.append("\nUsers without REDCap accounts → log them in the SPR (PID 221) via `redcap-admin`.")
    ...
    L.append("\n## Mark the tracker (token-optional)\n```bash\npython3 .../skills/redcap-build/sir_update.py %s --pid <PID> --mark-step project_created\n```\nNo token → set these `build_tracking` fields in the Study Tracker UI." % a.rid)
