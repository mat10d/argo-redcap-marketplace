...
```bash
CORE=$(find /mnt/.remote-plugins /mnt/skills ~/mnt ~/.claude/plugins -name argo_redcap_client.py 2>/dev/null | head -1)
...
```bash
B=$(find /mnt/.remote-plugins /mnt/skills ~/mnt ~/.claude/plugins -name backfill_sir_from_csv.py 2>/dev/null | head -1); python3 "$B" --csv /nonexistent.csv 2>&1 | tail -12
```
...
```bash
R=$(find /mnt/.remote-plugins /mnt/skills ~/mnt ~/.claude/plugins -name review_responses.py 2>/dev/null | head -1); printf 'not a real xlsx' > /tmp/corrupt.xlsx; python3 "$R" /tmp/missing.xlsx /tmp/other.xlsx 2>&1 | tail -6; echo "--- corrupt file ---"; python3 "$R" /tmp/corrupt.xlsx /tmp/corrupt.xlsx 2>&1 | tail -7
```
...
```bash
S=$(find /mnt/.remote-plugins /mnt/skills ~/mnt ~/.claude/plugins -name sir_update.py 2>/dev/null | head -1); python3 -c "
...
```bash
CORE=$(find /mnt/.remote-plugins /mnt/skills ~/mnt ~/.claude/plugins -name argo_redcap_client.py 2>/dev/null | head -1); ls -l "$CORE"; python3 -c "
