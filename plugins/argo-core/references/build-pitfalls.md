...
This is the running list of mistakes we've made on real ARGO builds and the rules we now follow to avoid repeating them. Reference from `redcap-build`, `redcap-admin`, and `study-intake`.
...
PIs often submit before IRB approval lands ("Pending" `irb_number`, blank `irb_approval_expires`). Backfill the SIR record once the cert is received — see `sir_update.py` in study-intake. Future analyses pulling the SIR for tracker context need the corrected values.
