...
Check your setup at any time (the client finds the settings file itself):
...
def load_env_file(explicit: str | None = None) -> "Path | None":
    ...
    In a terminal you can also do this yourself with `set -a; source <settings file>; set +a`.
...
def check_url(url: str | None) -> str:
    ...
    if not url:
        raise RedcapError(
            ...
            "it if you don't have it. Once you have it, open your ARGO settings file (double-click\n"
            "'Add keys here' in your ARGO folder) and add it on the REDCAP_URL line:\n"
            "\n"
    ...
    if parsed.scheme not in ("http", "https") or not parsed.netloc:
        raise RedcapError(
            ...
            "    REDCAP_URL=https://your-redcap-site.org/api/\n"
            "\n"
            "Fix the REDCAP_URL line in your ARGO settings file (double-click 'Add keys here'\n"
            "in your ARGO folder to open it)."
        )
    return url
    ...
    def _post(self, raw: bool = False, **params) -> "list | dict | str":
        ...
        raise RedcapError(
            ...
            + (_egress_blocked_message(self.url) if _looks_sandboxed() else
               "Check that you're connected to the internet, and that the REDCAP_URL line\n"
               "in your ARGO settings file is correct. If both look fine, REDCap itself may\n"
               "be down — try again in a few minutes.")
        ) from last_error
    ...
    def confirm_project(self, expect_title: str | None = None, expect_pid: str | None = None) -> dict:
        ...
        if expect_pid and str(expect_pid).strip() != actual_pid:
            raise ProjectMismatch(
                ...
                "\n"
                "The access key in your settings file points at a different REDCap project than\n"
                "the one this task is meant to change. Nothing has been modified."
            )
        ...
        if expect_title:
            ...
            if want and want in got:
                print(
                    ...
                    f"meant, tell me to stop before anything is written.",
                    file=sys.stderr,
                )
                return info
            raise ProjectMismatch(
                ...
                "\n"
                "The access key in your settings file points at a different REDCap project than\n"
                "the one this task is meant to change. Nothing has been modified. Check your\n"
                "settings file, or ask your REDCap administrator which key belongs to which\n"
                "project."
            )
        return info
...
def run_check() -> int:
    ...
    if study_keys:
        ...
        for name in study_keys:
            ...
            try:
                ...
                broken += 1
        print("  (Study keys open patient data — keep this folder private, and connect it\n"
              "   only to your own sessions.)")
    ...
    if broken:
        ...
        return 1
    if working == 0:
        where = (f"paste them into your settings file:\n    {found}"
                 if found else
