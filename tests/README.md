...
| File | Guards against |
|---|---|
| `test_pure_logic.py` | The logic that decides what the dashboards say and what the safety gates allow: URL validation, token masking, project-title matching, SIR build progress (`N/7`), week-over-week diffing, record-ID range parsing, and the QA dry-run receipt system. Also `export.py` end to end against a fake project (both encodings + the de-identified copies + the folder README), its record counting (CSV rows, not physical lines), its settings-file-anchored `--out`, and the security rule that `--check` only ever tries access keys written in the settings file it loaded — never `*_TOKEN` swept out of the environment. |
| `test_docs_match_code.py` | Documentation drifting away from code, and scripts crashing instead of explaining themselves. Includes the self-load guard: no script may read `REDCAP_URL` at import, and any script that reads it must load the ARGO settings file itself. |
...
| `test_linkage_merge.py` | Analyst/DB-manager task: LINKING two studies. Drives `link-data`'s three tools across `synthetic-study` and `synthetic-study-b` (which share the `syn_id` space). `link_studies.py`: the join-key survey ranks `syn_id` above `hospital_no` (45 matches vs 38), the hard link is 45 rows and exactly two columns, the two `*_missing_link.csv` files are 15 and 155 with names on them, and the name-review table is the 3 engineered discrepancies worst-first. `diff_payload.py`: fills, conflicts, no-ops and the no-record-to-fill report against study B's `MANIFEST.json` — the defect it used to pin was fixed in 0.17.2 — plus `--for-analysis` file names and no push language. `build_master_linkage.py`: 215 rows, 45 linked, 15 right-only, 155 left-only, plus the worst-first integrity report. |
